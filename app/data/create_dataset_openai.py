...
output_file = "app/data/dataset.json"
num_examples = 40
...
def create_system_prompt(
    ...
    base_prompt = (
        ...
        "스키마 설명입니다:\n"
        "- ID: 대화 식별자 (예: 'dialog_1')\n"
        "- turn_id: 현재 대화의 턴 번호 (1부터 시작)\n"
        "- domains: 현재 대화에서 다루는 도메인 리스트\n"
        "- dialog: 대화 내용\n"
        "  - sys: 시스템 발화 리스트 (첫 턴에서는 반드시 빈 문자열(''))\n"
        "  - usr: 사용자 발화 리스트\n"
        "- slot_values: 대화 시작부터 현재까지 누적된 모든 슬롯 값들\n"
        "- turn_slot_values: 현재 턴에서 새롭게 변경된 슬롯 값들\n"
        "- last_slot_values: 이전 턴에서 변경된 슬롯 값들\n\n"
        "슬롯 값 기록 시 주의사항:\n"
        "1. turn_slot_values는 현재 턴에서 변경된 값만 포함\n"
